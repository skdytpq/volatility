...
class NBeatsNet(nn.Module):
    ...
    def forward(self, backcast):
        ...
        backcast = torch.matmul(backcast.T,backcast).unsqueeze(0)
...
class Block(nn.Module):
    ...
    def __init__(self, units, thetas_dim, device, backcast_length=10, forecast_length=5, share_thetas=False,
                 nb_harmonics=None,drop_rate = 0.5):
        ...
        self.fc1 = nn.Linear(backcast_length, units) # nn.Linear to Attention or Convolution
...
class GenericBlock(Block):
    ...
    def forward(self, x):
        ...
        theta_b = self.dropout(theta_b)
        theta_f = self.theta_f_fc(x)
        theta_f = self.dropout(theta_f)
