...
class Decoder(nn.Module):
    ...
    def forward(self, hidden, cell):
        ...
        for _ in range(self.tau): # 0 ~ 47 인덱스까지 작업 총 48
...
class SQF_RNN(nn.Module):
    def __init__(self, d_input, d_model, tau, n_layers, M, device, dr=0.05):
        ...
        self.num_class = 14
        self.num_dim = 5
        self.embedding = nn.Embedding(self.num_class, self.num_dim)
    ...
    def forward(self, x, idx):
       # onehot_embed = self.embedding(idx).repeat(1,self.tau,1) #
       # x = torch.cat((x,onehot_embed),dim = 2)
        en_hidden, en_cell = self.encoder(x) # initi -> #layer,batch,d_model
        
        dec_output = self.decoder(en_hidden, en_cell)
        beta = self.softplus(self.beta_map(dec_output)) # 양수가 나오므로 단조 증가 함수 보장
        # batch X Len X d_input*(M+1)
        gamma = self.gamma_map(dec_output)    
        # batch X Len X d_input
        return gamma.view(-1, self.tau, self.d_input, 1), beta.view(-1, self.tau, self.d_input, self.M+1), self.delta
    ...
    def quantile_function(self, alpha, gamma, beta,delta): # prediction에서의 결과 값
       # pdb.set_trace() # batchX48X5X1 [64, 48, 5, 48] -> 64 48 5 1
        pdb.set_trace()
        return gamma + (beta * torch.where(
            alpha.unsqueeze(-1) - delta > 0,
            alpha.unsqueeze(-1) - delta,
            torch.zeros(()).to(self.device)
            )).sum(dim=-1, keepdims=True).squeeze(-1)
...
def train(model, loader, criterion, optimizer, device):
    ...
    for batch in loader:
        ...
        predict = model.quantile_function(alpha_tilde, gamma, beta)
...
class LinearSplineCRPS(nn.Module): # Loss function
